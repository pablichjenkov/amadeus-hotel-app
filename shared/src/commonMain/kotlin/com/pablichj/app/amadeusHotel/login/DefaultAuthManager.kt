...
class DefaultAuthManager(authorizeAPI: AuthorizeAPI) : AuthManager {
    ...
    override fun requestRecoverPassword(
        ...
        val recoverPasswordRequestStatus = MutableStateFlow<RecoverPasswordRequestStatus>(
            RecoverPasswordRequestStatus.RecoverPasswordInProgress
        )
        coroutineScope.launch {
            delay(3000)
            token = "fake_token"
            recoverPasswordRequestStatus.value =
                RecoverPasswordRequestStatus.RecoverPasswordSuccess()
            cancel(null)
        }
